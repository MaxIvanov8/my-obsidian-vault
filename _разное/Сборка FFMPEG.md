...
- `--disable-avdevice`, `--disable-swresample`, `--disable-postproc`, `--disable-avfilter`: Отключает целые подсистемы, которые не нужны для базовой нарезки видео
...
**Установка в локальную папку**: После успешной компиляции (это может занять несколько минут), выполняем "установку", которая скопирует все нужные файлы в папку `build_out`, указанную ранее.
```
make install
```
