...
**Запуск компиляции**: Если предыдущая команда выполнилась без ошибок (в конце она выведет список включенных компонентов), запускаем сборку
```
make -j$(nproc)
```
- `-j$(nproc)` (или `-j8`): Указывает компилятору использовать все доступные ядра процессора для ускорения процесса.
